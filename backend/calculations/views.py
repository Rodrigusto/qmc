...
def calculation_list(request):
    calculations = CostCalculation.objects.select_related("product").order_by(
        "-created_at"
    )
    context = {
...
def calculation_new(request):
    ...
    if request.method == "POST":
        ...
        product = Product.objects.prefetch_related("recipe_items__ingredient").get(
            pk=product_id
        )
        ingredient_cost = product.ingredient_cost

        total_fixed = (
            FixedCost.objects.filter(is_active=True).aggregate(
...
def forecast_list(request):
    ...
    now = datetime.date.today()
    forecasts = SalesForecast.objects.select_related("product").order_by(
        "-year", "-month"
    )
    return render(
        request,
        "calculations/forecast_list.html",
        {
            "forecasts": forecasts,
...
def forecast_new(request):
    ...
    return render(
        request,
        "calculations/forecast_new.html",
        {
            "products": products,
            "current_month": now.month,
            "current_year": now.year,
