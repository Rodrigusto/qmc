...
class CostCalculation(BaseModel):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.PROTECT, related_name="calculations"
    )
    expected_monthly_sales = models.PositiveIntegerField(
        "Vendas mensais esperadas", validators=[MinValueValidator(1)]
    )
...
class SalesForecast(BaseModel):
    ...
    month = models.PositiveIntegerField(
        "Mês", choices=[(i, str(i)) for i in range(1, 13)]
    )
    year = models.PositiveIntegerField("Ano")
    expected_units = models.PositiveIntegerField(
        "Unidades previstas", validators=[MinValueValidator(1)]
    )
