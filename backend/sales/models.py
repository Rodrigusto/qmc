...
class Neighborhood(BaseModel):
    ...
    delivery_fee = models.DecimalField(
        "Taxa de entrega (R$)", max_digits=8, decimal_places=2, default=0
    )
...
class SalesChannel(BaseModel):
    ...
    fee_type = models.CharField(
        "Tipo de taxa", max_length=10, choices=FeeType.choices, default=FeeType.NONE
    )
    ...
    def calculate_fee(self, subtotal: Decimal) -> Decimal:
        ...
        if self.fee_type == self.FeeType.PERCENT:
            return subtotal * (to_decimal(self.fee_value) / 100)
        elif self.fee_type == self.FeeType.FIXED:
            return to_decimal(self.fee_value)
        return Decimal("0")
...
class Sale(BaseModel):
    ...
    status = models.CharField(
        "Status", max_length=12, choices=Status.choices, default=Status.CONFIRMED
    )
    discount = models.DecimalField(
        "Desconto (R$)", max_digits=8, decimal_places=2, default=0
    )
    note = models.TextField("Observações", blank=True)
    ...
    def recalculate_totals(self):
        ...
        self.delivery_fee = (
            to_decimal(self.neighborhood.delivery_fee)
